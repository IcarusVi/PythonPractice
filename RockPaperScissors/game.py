...
class Game:
    ...
    def startGame(self):
        gameType = input(
            'Choose a game type: 1(Player vs Player) or 2(Player vs Bot)')
        # If game type = 1 initiate PVP game, if 2 initiate Player vs Bot game
        self.pvpGame(gameType)

    # Function that maintains the loop until a player quits
    def gameLoop(self, playerOne, playerTwo):
    ...
    def pvpGame(self, gameType):
        if gameType == '1':
            ...
            while self.resume:
                self.rounds += 1
                self.gameLoop(playerOne, playerTwo)
    ...
    # Function to determine the winner from a round
